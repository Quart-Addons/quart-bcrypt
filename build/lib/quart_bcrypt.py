...
async def async_generate_password_hash(password, rounds=None):
...
async def async_check_password_hash(pw_hash, password):
    """This async helper function wraps the eponymous method of :class:`Bcrypt.` It 
    is intended to be used as a helper function at the expense of the 
    configuration variable provided when passing back the app object. In other 
    words this shortcut does not make use of the app object at all.
...
class Bcrypt(object):
    ...
    **Warning: if this option is enabled on an existing project, disabling it
    ...
    async def async_generate_password_hash(self, password, rounds=None, prefix=None):
    ...
    async def async_check_password_hash(self, pw_hash, password):
