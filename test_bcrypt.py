...
class AsyncBasicTestCase(aiounittest.AsyncTestCase):
    ...
    async def test_is_string(self):
        pw_hash = await self.bcrypt.async_generate_password_hash('secret')
        self.assertTrue(isinstance(pw_hash, bytes))

    async def test_custom_rounds(self):
        password = 'secret'
        pw_hash1 = await self.bcrypt.async_generate_password_hash(password, 5)
        self.assertNotEqual(password, pw_hash1)

    async def test_check_hash(self):
        pw_hash = await self.bcrypt.async_generate_password_hash('secret')
        # check a correct password
        self.assertTrue(await self.bcrypt.async_check_password_hash(pw_hash, 'secret'))
        # check an incorrect password
        self.assertFalse(await self.bcrypt.async_check_password_hash(pw_hash, 'hunter2'))
        # check unicode
        pw_hash = await self.bcrypt.async_generate_password_hash(u'\u2603')
        self.assertTrue(await self.bcrypt.async_check_password_hash(pw_hash, u'\u2603'))
        # check helpers
        pw_hash = await async_generate_password_hash('hunter2')
        self.assertTrue(await async_check_password_hash(pw_hash, 'hunter2'))

    async def test_check_hash_unicode_is_utf8(self):
        password = u'\u2603'
        pw_hash = await self.bcrypt.async_generate_password_hash(password)
        # check a correct password
        self.assertTrue(await self.bcrypt.async_check_password_hash(pw_hash, b'\xe2\x98\x83'))

    async def test_unicode_hash(self):
        password = u'東京'
        h = await async_generate_password_hash(password)
        self.assertTrue(await async_check_password_hash(h, password))
    ...
    async def test_long_password(self):
        ...
        pw_hash = await self.bcrypt.async_generate_password_hash(password)
        # Ensure that a longer password yields the same hash
        self.assertTrue(await self.bcrypt.async_check_password_hash(pw_hash, 'A' * 80))

class AsyncLongPasswordsTestCase(AsyncBasicTestCase):
    ...
    async def test_long_password(self):
        ...
        pw_hash = await self.bcrypt.async_generate_password_hash(password)
        # Ensure that a longer password **do not** yield the same hash
        self.assertFalse(await self.bcrypt.async_check_password_hash(pw_hash, 'A' * 80))